...
class KelasController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(Session::get('token')){            
            try{           
                $data = Http::withToken(Session::get('token'))->get($this->api_host.'/api/kelas')->json();
                
                $table = $this->getPaginator($data['data'], $request);
                return view('kelas.index', compact('table') );
            }
            catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        if(Session::get('token')){  
            try{   
                $response = Http::withToken(Session::get('token'))->post($this->api_host.'/api/kelas', [
    ...
    public function edit(Request $request, $id)
    {
        if(Session::get('token')){  
            try{         
                ...
                return view('kelas.edit',compact('table'));
            }
            catch (\Exception $e) {
                return redirect()->route('error-404'); 
            }
